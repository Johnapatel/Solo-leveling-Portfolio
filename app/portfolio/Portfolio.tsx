...
const clips = ["After the dark ARISE", "A quiet kind of power", "Find the rhythm", "Into another world", "Eyes on the story", "One last frame"];
...
function Loop({n,motion,className=""}:{n:number;motion:boolean;className?:string}){
 ...
 return <video ref={ref} className={className} src={`/media/loop-${n}.mp4`} muted loop playsInline preload="metadata" aria-label={`Video edit sample ${n}`}/>;
}
